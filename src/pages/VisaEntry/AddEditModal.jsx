...
export function AddEditModal({ showModal, closeModal, onRefreshVisaEntry }) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const { postData, patchData, isLoading } = useVisaEntryReducer(
    (state) => state
  );
  ...
  const renderHeader = () => (
    <>
      <h4 className="modal-title">
        {showModal?.id ? 'Edit Visa Entry' : 'Add Visa Entry'}
      </h4>
      <button
        type="button"
        class="btn-close"
