...
export function AddEditModal({ showModal, closeModal, onRefreshApplicationType }) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const { postData, patchData, isLoading } = useApplicationTypeReducer(
    (state) => state
  );
  ...
  const renderHeader = () => (
    <>
      <h4 className="modal-title">
        {showModal?.id ? 'Edit Type' : 'Add Type'}
      </h4>
      <button
        type="button"
        class="btn-close"
