...
export function AddEditModal({ showModal, closeModal, onRefreshVisaService }) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const { postData, patchData, isLoading } = useVisaServiceReducer(
    (state) => state
  );
  ...
  const renderHeader = () => (
    <>
      <h4 className="modal-title">
        {showModal?.id ? 'Edit Visa Service' : 'Add Visa Service'}
      </h4>
      <button
        type="button"
        class="btn-close"
