...
export function AddEditModal({ showModal, closeModal, onRefreshVisaDuration }) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const { postData, patchData, isLoading } = useVisaDurationReducer(
    (state) => state
  );
  ...
  const renderHeader = () => (
    <>
      <h4 className="modal-title">
        {showModal?.id ? 'Edit Visa Duration' : 'Add Visa Duration'}
      </h4>
      <button
        type="button"
        class="btn-close"
